...
async function answerPressureComparisonDebrief(page: Page, confidence: number, effort: 'Clearer' | 'Unchanged') {
...
test.describe('V1.2 active workspace visual contract @visual', () => {
  ...
  test('Pressure comparison brief', async ({ page }, testInfo) => {
    await page.setViewportSize({ width: 1100, height: 1000 })
    await page.goto('/#lab/pressure')
    await page.getByRole('button', { name: 'Run controlled comparison' }).click()
    await disableNonessentialMotion(page)
    await expectVisual(page.locator('.pressure-comparison'), 'pressure-comparison-brief', testInfo)
  })

  test('Pressure comparison completed results', async ({ page }, testInfo) => {
    await page.clock.install()
