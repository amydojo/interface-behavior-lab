...
async function answerDebrief(
  page: Page,
  confidence: number,
  effort: 'Easier' | 'Unchanged',
  assistance: 'Helpful' | 'Neutral',
) {
...
test.describe('Magnetic comparison visual contract @visual', () => {
  test('Magnetic comparison brief', async ({ page }, testInfo) => {
    ...
    await expectVisual(page.locator('.magnetic-comparison'), 'magnetic-comparison-brief', testInfo)
  })

  test('Magnetic comparison completed results', async ({ page }, testInfo) => {
    ...
    await adaptiveTarget.focus()
    await page.keyboard.press('Enter')
    await answerDebrief(page, 5, 'Easier', 'Helpful')
    await disableNonessentialMotion(page)

    await expectVisual(
      page.locator('.magnetic-comparison'),
      'magnetic-comparison-results',
      testInfo,
      [page.locator('.magnetic-result-grid article > strong')],
    )
  })
})
